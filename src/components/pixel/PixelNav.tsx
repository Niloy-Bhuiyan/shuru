...
export function PixelNav() {
  ...
  return (
    <nav
      aria-label="Main"
      className="fixed inset-x-0 bottom-0 z-40 mx-auto max-w-app border-t border-ui-line bg-paper pb-[env(safe-area-inset-bottom)] lg:hidden"
    >
        ...
        {ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              className={cx(
                "flex flex-col items-center gap-1 py-2 pt-1.5 font-sans text-[11px] font-medium transition-colors",
                active ? "text-ink" : "text-ui-faint"
              )}
            >
              <span
                aria-hidden
                className={cx("h-[3px] w-8 rounded-full", active ? "bg-amber" : "bg-transparent")}
              />
